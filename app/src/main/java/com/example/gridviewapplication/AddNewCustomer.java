...
public class AddNewCustomer extends AppCompatActivity {
    ...
    private EditText et_f_name, et_company_name, et_l_name, et_mobile, et_address, et_opening_balance, et_gst_number;
...
//        });
        ...
        save.setOnClickListener(v -> {
            ...
            if (canClick) {
                String f_name, company_name, l_name, mobile, address, gst_number;
                f_name = String.valueOf(et_f_name.getText());
                company_name = String.valueOf(et_company_name.getText());
                l_name = String.valueOf(et_l_name.getText());
                mobile = String.valueOf(et_mobile.getText());
                address = String.valueOf(et_address.getText());
                gst_number = String.valueOf(et_opening_balance.getText());

                //if ((!f_name.equals("")) && (!l_name.equals("")) && (mobile.length() == 10) && (!address.equals("")) && (Integer.parseInt(opening_balance) > 0)) {
                if ((!f_name.equals("")) && (!l_name.equals("")) && (mobile.length() == 10) && (!address.equals(""))) {
                    Log.e("values", "" + f_name + company_name + l_name + mobile + address + gst_number);
                    postCustomer(f_name, company_name, l_name, mobile, address, gst_number);
                } else {
    ...
    private void init() {
        ...
        et_company_name = binding.contentAddNewCustomer.etCompanyName;
        et_l_name = binding.contentAddNewCustomer.etLName;
        et_mobile = binding.contentAddNewCustomer.etMobile;
        et_gst_number = binding.contentAddNewCustomer.etGstNumber;
...
//    }
    ...
    private void postCustomer(String f_name, String company_name, String l_name, String mobile, String address, String gst_number) {
        ...
        try {
            ...
            jsonObj.put("cust_m_name", "");
            jsonObj.put("cust_l_name", l_name);
            jsonObj.put("cust_ph_no", mobile);
            jsonObj.put("cust_address", address);
            jsonObj.put("cust_balance", "0");
            jsonObj.put("cust_app_limit", "100000");
            jsonObj.put("cust_id", JSONObject.NULL);
            jsonObj.put("cust_post_paid", "Y");
            jsonObj.put("cust_company", company_name);
            jsonObj.put("cust_gst", gst_number);
            jsonObj.put("cust_service", "0");
            jsonObj.put("cust_type", "new");

            jsonObj.put("cust_outstanding", "0");
            jsonObj.put("cust_credit_limit", "200000");
